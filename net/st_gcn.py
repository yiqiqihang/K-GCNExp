...
class Model(nn.Module):
    ...
    def forward(self, x):
        # data normalization
        N, C, T, V, M = x.size()
        # 网络的输入(N = batch_size,C = 3,T = frame_num,V = 25(node_num),M = 2)
        ...
        x = x.view(N * M, C, T, V)
        # 注意，这里网络输入将 N, C, T, V, M整合成了N，C，V，T。将batch和person_num维度整合了一起
...
class st_gcn(nn.Module):
    ...
    Args:
        ...
        kernel_size (tuple): Size of the temporal convolution kernel and graph convolution kernel
