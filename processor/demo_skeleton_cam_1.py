...
class DemoSkeletonCam(Processor):
        ...
        if self.arg.data_level == 'instance':
            ...
            for data, label in loader:
                ...
                if self.method == "bicam":
                    ...
                    saliency_map, model_output, pred_class = cam_func(data)
                # ave_drop, incr, inse_auc, del_auc = self.evaluate(data, saliency_map, label_1)
    ...
    def saliency_map(self, shapley_value, data):
        ...
        left_hand, right_hand, left_leg, right_leg, trunk, unique_keys_q3, cutqvalue, left_hand_right_hand_trunks, left_leg_right_legs = self.cut_skeleton(
            data)
        shapley_value = np.array(shapley_value).squeeze(1)
        ...
        if self.model_count == 0:
            ...
            saliency_map = cutqvalue[self.q3_i][0] * float(shapley_value[0][1]) + cutqvalue[self.q3_i][1] * float(
                shapley_value[1][1]) + \
                           cutqvalue[self.q3_i][2] * float(shapley_value[2][1])
        ...
        base_saliency_map = self.base_cam(data)
        base_saliency_map = base_saliency_map.cpu().to(self.dev)
        saliency_map = torch.sum(saliency_map, dim=1, keepdim=True).float().to(self.dev)
        saliency_map = saliency_map * saliency_map
        saliency_map = saliency_map * base_saliency_map
        saliency_map = self.data_norm(F.relu(saliency_map))
        ...
        # saliency_map = self.data_norm(F.sigmoid(saliency_map))
        return saliency_map
    ...
    def ShapleyCam(self, data, label):
        ...
        left_hand, right_hand, left_leg, right_leg, trunk, unique_keys_q3, cutqvalue, left_hand_right_hand_trunks, left_leg_right_legs = self.cut_skeleton(
            data)
        # print(unique_keys_q3[self.q3_i][0] + ',' + unique_keys_q3[self.q3_i][1] + ',' + unique_keys_q3[self.q3_i][2])
        unique_keys_q_2 = ['left_hand_right_hand_trunk', 'left_leg_right_leg']
        source_data = self.permutation_combination(data, label, left_hand, right_hand, left_leg, right_leg, trunk,
                                                   unique_keys_q3, cutqvalue, left_hand_right_hand_trunks,
                                                   left_leg_right_legs)
        if self.model_count == 0:
            shapley_value = self.compute_shapley_values(unique_keys, source_data)
        elif self.model_count == 1:
            shapley_value = self.compute_shapley_values(unique_keys_q3[self.q3_i], source_data)
        elif self.model_count == 2:
            shapley_value = self.compute_shapley_values(unique_keys_q_2, source_data)
    ...
    def cut_skeleton(self, data):
        ...
        # 3
        unique_keys_q2, unique_keys_q3, value = key.outofkeyw(data)
        # unique_keys_q3 zuhe
        # cutqvalue  cut_result

        # 2
        unique_keys_q_2 = ['left_hand_right_hand_trunk', 'left_leg_right_leg']
        left_hand_right_hand_trunk = np.zeros((25, 3))
        left_hand_right_hand_trunk[[8, 9, 10, 11, 23, 24, 4, 5, 6, 7, 21, 22, 0, 1, 2, 3, 20], :] = 1
        left_hand_right_hand_trunks = np.tile(left_hand_right_hand_trunk, (T * M, 1))
        left_hand_right_hand_trunks = torch.from_numpy(left_hand_right_hand_trunks)
        left_hand_right_hand_trunks = self.data_reshape(data, left_hand_right_hand_trunks)

        left_leg_right_leg = np.zeros((25, 3))
        left_leg_right_leg[[16, 17, 18, 19, 12, 13, 14, 15], :] = 1
        left_leg_right_legs = np.tile(left_hand_right_hand_trunk, (T * M, 1))
        left_leg_right_legs = torch.from_numpy(left_leg_right_legs)
        left_leg_right_legs = self.data_reshape(data, left_leg_right_legs)

        return left_hands, right_hands, left_legs, right_legs, trunks, unique_keys_q3, value, left_hand_right_hand_trunks, left_leg_right_legs


    def permutation_combination(self, data, label, left_hand, right_hand, left_leg, right_leg, trunk, unique_keys_q3, cutqvalue,left_hand_right_hand_trunks,left_leg_right_legs):
        """
        ...
        if (self.model_count == 0):
            ...
            with torch.no_grad():
                three_four_five_out = self.model(three_four_five)
                probability_three_four_five = (torch.nn.functional.softmax(three_four_five_out[0], dim=-1))[
                    label].item()
            source_data["left_leg,right_leg,trunk"] = probability_three_four_five
            # </editor-fold>
            ...
            for i in range(3):
                data = data.to(self.dev)
                temp = cutqvalue[self.q3_i][i].to(self.dev)
                temp = (data * temp).float().to(self.dev)
                with torch.no_grad():
                    temp_out = self.model(temp)
                    probability_temp = (torch.nn.functional.softmax(temp_out[0], dim=-1))[
                        label].item()

                source_data[unique_keys_q3[self.q3_i][i]] = probability_temp

            for i in range(3):
                for y in range(3):
                    if y == 0:
                        temp = (cutqvalue[self.q3_i][0] + cutqvalue[self.q3_i][1]).to(self.dev)
                    elif y == 1:
                        temp = (cutqvalue[self.q3_i][0] + cutqvalue[self.q3_i][2]).to(self.dev)
                    elif y == 2:
                        temp = (cutqvalue[self.q3_i][1] + cutqvalue[self.q3_i][2]).to(self.dev)

                    # print("--------")
                    # print(cutqvalue[self.q3_i][1] + cutqvalue[self.q3_i][2])
                    # print("--------")
                    ...
                    if y == 0:
                        source_data[
                            unique_keys_q3[self.q3_i][0] + ',' + unique_keys_q3[self.q3_i][1]] = probability_temp
                    elif y == 1:
                        source_data[
                            unique_keys_q3[self.q3_i][0] + ',' + unique_keys_q3[self.q3_i][2]] = probability_temp
                    elif y == 2:
                        source_data[
                            unique_keys_q3[self.q3_i][1] + ',' + unique_keys_q3[self.q3_i][2]] = probability_temp

            # 3
            data = data.to(self.dev)
            temp = (cutqvalue[self.q3_i][0] + cutqvalue[self.q3_i][1] + cutqvalue[self.q3_i][2]).to(self.dev)
            temp = (data * temp).float().to(self.dev)
            with torch.no_grad():
                temp_out = self.model(temp)
                probability_temp = (torch.nn.functional.softmax(temp_out[0], dim=-1))[
                    label].item()
            source_data[
                unique_keys_q3[self.q3_i][0] + ',' + unique_keys_q3[self.q3_i][1] + ',' + unique_keys_q3[self.q3_i][
                    2]] = probability_temp
            ...
            # 1
            data = data.to(self.dev)
            left_hand_right_hand_trunks = left_hand_right_hand_trunks.to(self.dev)
            left_hand_right_hand_trunk = (left_hand_right_hand_trunks * data).float().to(self.dev)
            with torch.no_grad():
                left_hand_right_hand_trunk_out = self.model(left_hand_right_hand_trunk)
                probability_left_hand_right_hand_trunk = \
                (torch.nn.functional.softmax(left_hand_right_hand_trunk_out[0], dim=-1))[label].item()
            source_data["left_hand_right_hand_trunk"] = probability_left_hand_right_hand_trunk

            left_leg_right_legs = left_leg_right_legs.to(self.dev)
            left_leg_right_leg = (left_leg_right_legs * data).float().to(self.dev)
            with torch.no_grad():
                left_leg_right_leg_out = self.model(left_leg_right_leg)
                probability_left_leg_right_leg = (torch.nn.functional.softmax(left_leg_right_leg_out[0], dim=-1))[
                    label].item()
            source_data["left_leg_right_leg"] = probability_left_leg_right_leg

            # 2
            data = data.to(self.dev)
            totall = ((left_hand_right_hand_trunks + left_leg_right_legs) * data).float().to(self.dev)
            with torch.no_grad():
                totall_out = self.model(totall)
                probability_totall = (torch.nn.functional.softmax(totall_out[0], dim=-1))[label].item()
            source_data["left_hand_right_hand_trunk,left_leg_right_leg"] = probability_totall
    ...
    def GradCampp(self, data, action_class=None):
        ...
        if self.arg.data_level == "test_set":
            assert self.arg.test_batch_size == 1, "Test batch size for GradCAM++ method must be one."
        # inference
        self.model = self.model.double().to(self.dev)
        self.model.eval()
        self.model.zero_grad()
