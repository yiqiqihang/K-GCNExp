...
from Model_crcam import load_model, load_test, load_test_single,load_test_plot#解释
import time
...
def plot(data,saliency_map):
    ...
    for j in range(0,len(Long1),cut):
        f = fea[j, :]
    ...
    for j in range(0,len(Long2),cut):
        f = fea[j, :]
           
        ax.scatter3D(La2[j],Long2[j],A2[j], color='black', s=40,marker='^')
        ax.scatter3D(La2[j],Long2[j],A2[j], color='red', s=40,marker='o',alpha=f[1, 0].item())

    # figure0 = ax.scatter(La1[::cut], Long1[::cut], A1[::cut], c=color, marker='^', s=2)
    # figure1 = ax.scatter(La2[::cut], Long2[::cut], A2[::cut], c=color, marker='^', s=2)
    # ax.scatter3D(z,x,y, color='red', s=20)
    # ax.set_xlabel(' time\n----->', fontsize=20)
    ax.set_ylabel(' time\n----->', fontsize=20)
    ax.set_xticks([])
    ax.set_yticks([])
...
if __name__ == '__main__':
    ...
    data_path = "/data/home/st/GT_CAM/st-gcn/test/rechange_val_data_plane_Radar.npy"
    label_path = "/data/home/st/GT_CAM/st-gcn/test/rechange_val_label_plane_Radar.pkl"
    ...
    visoutput_result_dir =f'./result/visualization/gtcam-{time_now}/'
    ...
    for index in indices:
        saliencymap,Typelabel,Typelabel1 = load_test_plot(all_data,label,index,time_now)
