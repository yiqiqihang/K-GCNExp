...
# 加载预训练模型的特征提取部分
model_args = {'in_channels': 3, 'num_class': 3, 'dropout': 0, 'edge_importance_weighting': False,
              'graph_args': {'layout': 'ntu-rgb+d', 'strategy': 'spatial'}} 


#双机特征5维度(1, 5, 1, 1, 1)，五机特征3维度，船只特征2维度

# pre_model_path =  r".\models\epoch81_best_model.pt"
pre_model_path = r"/data/home/yr/YRR/suweipeng/test_fiveairplanes-v0.1/models/epoch81_best_model.pt"
...
# def load_test(all_data,label):
    ...
    for index in range(len(all_data["Longitude1"])):
        ...
        long1 = all_data["Longitude1"][index].copy()
        long2 = all_data["Longitude2"][index].copy()
        long3 = all_data["Longitude3"][index].copy()
        long4 = all_data["Longitude4"][index].copy()
        long5 = all_data["Longitude5"][index].copy()
        
        la1 = all_data["Latitude1"][index].copy()
        la2 = all_data["Latitude2"][index].copy()
        la3 = all_data["Latitude3"][index].copy()
        la4 = all_data["Latitude4"][index].copy()
        la5 = all_data["Latitude5"][index].copy()
        a1 = all_data["Altitude1"][index].copy()
        a2 = all_data["Altitude2"][index].copy()
        a3 = all_data["Altitude3"][index].copy()
        a4 = all_data["Altitude4"][index].copy()
        a5 = all_data["Altitude5"][index].copy()


        
        data_val = np.zeros((1, 3, len(long1), 5, 1))

        for nums in range(min(len(long1),len(long2))):
            data_val[0, :, nums, 0, 0] = [long1[nums], la1[nums], a1[nums]]
            data_val[0, :, nums, 1, 0] = [long2[nums], la2[nums], a2[nums]]
            data_val[0, :, nums, 2, 0] = [long3[nums], la3[nums], a3[nums]]
            data_val[0, :, nums, 3, 0] = [long4[nums], la4[nums], a4[nums]]
            data_val[0, :, nums, 4, 0] = [long5[nums], la5[nums], a5[nums]]
            # data_numpy = np.array(data_val)
...
def load_test_noplot(all_data,label):
    accurate = 0
    total = 0
    ave_drop_all = 0
    ave_increase_all = 0
    ...
    file_txt.write("================================END==============================" + '\n')
        ...
        # # plt.close()
    print("total:",total)
    file_txt.write("总样本数" + str(total) + '\n')
    print("accurate:",accurate,"accurate/total:",accurate/total)
    file_txt.write("预测正确样本数：" + str(accurate) + "预测概率"+str(format(accurate/total, '.6f'))+'\n')
    # print(accurate/total)
# @torchsnooper.snoop()
...
def saliency_map1(shapley_value, data, output_org):
        ...
        base_saliency_map= GradCampp(output_org, data)
