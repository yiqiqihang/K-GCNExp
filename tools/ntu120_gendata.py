...
def gendata(data_path,
    ...
    for filename in os.listdir(data_path):
        if filename in ignored_samples:
            continue
        # print("----------------")
        # print(ignored_sample_path)
        # print(filename)
        action_class = int(
            filename[filename.find('A') + 1:filename.find('A') + 4])
        subject_id = int(
            filename[filename.find('P') + 1:filename.find('P') + 4])
        substring = filename[filename.find('S') + 1:filename.find('S') + 4]
        setup_id = int(''.join(filter(str.isdigit, substring)))

        # camera_id = int(
        #     filename[filename.find('C') + 1:filename.find('C') + 4])

        if benchmark == 'csetup':
            settraining_subjects = [19]
            # # istraining = [i for i in range(1, 33) if i % 2 == 0]
            # istraining = (setup_id in training_setup)
            # settraining_subjects = [i for i in range(1, 33) if i % 2 == 0]
            istraining = (setup_id in settraining_subjects)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--data_path', default='data/NTU-RGB-D/nturgb+d_120')
    parser.add_argument(
        '--ignored_sample_path',
        default='/devdata/home/st/GT_CAM/st-gcn/resource/NTU-RGB-D/NTU_RGBD120_samples_with_missing_skeletons.txt')
